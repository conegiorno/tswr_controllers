...
iter_icem = 12 # icem loop iterations
...
elite_frac = 0.3 # fraction of elites kept
cost_err = 0.1 # minimal cost that will break iCEM loop
...
class iCEMController(Node):
    ...
    def simulate_cost(self, model, state: State, target_state, dist, elites_kept):
        cost_tuple_list = []        
        # initial sim state
        x0 = np.array([state.x, state.y, state.yaw, state.v])

        if elites_kept != None:
            for steer_angle in elites_kept:
        ...
        for iteration in range(iter_icem):
            steer_angle = self.random_steer(mu, sigma)
            u_trj = np.zeros((traj_dim, 2), dtype=float)
            u_trj[:, 0] = state.a
            u_trj[:, 1] = steer_angle
            x_trj = np.zeros((u_trj.shape[0] + 1, x0.shape[0]))
    ...
    def keep_elites(self, cost_tuple_list, elite_frac):
        kept_size = int(len(cost_tuple_list) * elite_frac)
        elites_kept = list(zip(*cost_tuple_list))[1]
        elites_kept = elites_kept[0 : kept_size]
        return elites_kept
    ...
    def icem_steer_control(self, state: State, Lf):
        ...
        for iteration in range(iter_icem):
            ...
            cost_tuple_list = self.simulate_cost(self.model, state, target_state, Lf, elites_kept)
            cost_tuple_list.sort()
            # keep elites
            elites_kept = self.keep_elites(cost_tuple_list, elite_frac)
